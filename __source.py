...
class Remesher(bpy.types.Operator):
    ...
    def __init__(self):
        ...
        if obj.type == "MESH":
            ...
            self.edge_length_default = round((5 * avg_len) / 10, ndigits=2)
            self.edge_length = self.edge_length_default

    edge_length: bpy.props.FloatProperty(name="Edge Length", min=0, default=1)

    iterations: bpy.props.IntProperty(name="Iterations", min=1, default=30)

    quads: bpy.props.BoolProperty(name="Quads", default=True)
...
def draw(self, context):
    self.layout.operator(
        "remesh.boundary_aligned_remesh", text="Boundary Aligned Remesh"
    )
